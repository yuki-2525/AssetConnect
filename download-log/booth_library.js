...
const debugLog = (...args) => window.debugLogger?.log('[LIBRARY]', ...args);
...
function addAvatarExplorerDownloadButtons(root = document) {
    ...
    dropdowns.forEach(dropdown => {
        ...
        try {
            ...
            debugLog('Library: Failed to parse other download methods:', error);
            return;
        }
        ...
        avatarExplorerRow.addEventListener('click', async event => {
            ...
            try {
                ...
                if (!response?.success || !response.deeplink) {
                    window.debugLogger?.warn('[LIBRARY] AvatarExplorer deeplink failed:', response);
                ...
                if (regularDownloadButton) {
                    ...
                    saveDownloadHistory(info).catch(error => {
                        window.debugLogger?.warn('[LIBRARY] Download history save failed:', error);
                    });
                }
            } catch (error) {
                launchWindow?.close();
